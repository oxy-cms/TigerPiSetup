...
def main(): #main function
  ...
  call(['openssl', 'des3', '-d', '-in', '/home/pi/dd_script/nopass4u.txt', '-out', '/home/pi/dd_script/pwdfile.txt', '-pass', pwd]) #Here openssl is called to decrypt the pre-encrypted password file. Naming the decryped file as "pwdfile.txt" is required for the email setup later on. When encrypting a new password file, it is imperative that the same name ("nopass4u.txt") and encryption method ("des3") be used.
  if os.stat("/home/pi/dd_script/pwdfile.txt").st_size == 0: #for whatever reason openssl will create an empty file upon a bad decryption and not simply give an error. This if loop prevents the program from continuing in the event of a bad decryption
      call(['rm', '/home/pi/dd_script/pwdfile.txt']) #removes the empty file to prevent issues upon program restart in the event of a failed decryption
      ...
      def makescripts() #easier to group this as one function to create the shell scripts used to launch, restart, etc the web content. Saves lines of code/resources
        ...
        def makeshortcut() #subscript to autolaunch chrome on startup
            ...
            shortcut.write("Comment=by Enny Jole") #credit where credit is due (me)
        ...
        if os.path.isdir("/home/pi/.config/autostart/") == True: #tests to see if the autostart directory exists or not
            do makeshortcut() #if it does, then the shortcut function runs
        else: #if not--
            os.mkdir("/home/pi/.config/autostart/") #then we make it
            do makeshortcut() #and then run the shortcut function
      do makescripts() #actually runs the script making function
      #Email Setup
      ...
      if os.path.isdir(certdir) == True: #if the cert directory exists--
        do email_setup() #then the email setup goes through
      else: #otherwise
        call(['sudo', 'mkdir', certdir]) #we create it
        do email_setup() #then do the email setup
      do cronjobs() #then after do the cronjob
      do ipmail() #then sends IP
